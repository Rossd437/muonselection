...
def is_point_outside(point:np.ndarray, x_boundaries:np.ndarray, 
                     y_boundaries:np.ndarray, z_boundaries:np.ndarray) -> bool:
...
def check_intersection(traj:np.ndarray, min_bounds:np.ndarray, max_bounds:np.ndarray) -> bool:
    ...
    """
    direction = (traj['xyz_start']-traj['xyz_end'])/np.linalg.norm(traj['xyz_start']-traj['xyz_end'])
...
def detector_eff(f:h5flow.data.h5flow_data_manager.H5FlowDataManager) -> int:
    ...
    for muon_traj in muon_trajs:
        index_vertex = np.where(f['mc_truth/interactions/data']['vertex_id'] == muon_traj['vertex_id'])[0]
        try:
