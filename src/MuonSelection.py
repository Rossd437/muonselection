...
class MuonSelection:
    ...
    def __init__(
        ...
        track_count: int,
        length_cut: int,
        rock_muon_dtype: np.dtypes.VoidDType,
        segment_dtype: np.dtypes.VoidDType,
    ...
    def merge_test(
        self,
        main_cluster_direction: np.ndarray,
        main_cluster_mean: np.ndarray,
        ...
        distances = [
            self.average_distance(
                test_cluster, main_cluster_mean, main_cluster_direction
            )
            for test_cluster in test_clusters
        ]

        indices = [
            index for index, dist in enumerate(distances) if dist <= average_dist
        ]
    ...
    def cluster(self, PromptHits_ev: np.ndarray, average_dist: float) -> list:
        ...
        positions = np.column_stack(
            (PromptHits_ev["x"], PromptHits_ev["y"], PromptHits_ev["z"])
        )

        dbscan = DBSCAN(min_samples=6, eps=4 * 0.4434)
        ...
        for label in np.unique(non_noise_labels):
            indicies_of_clusters.append(np.where(non_noise_labels == label)[0])
        ...
        sorted_indices_of_cluster = [
            indicies_of_clusters[index] for index in sorted_indices_cluster_directions
        ]
        sorted_directions = [
            direction_each_cluster[index] for index in sorted_indices_cluster_directions
        ]

        mean_per_cluster = [
            np.mean(cluster, axis=0) for cluster in positions_per_cluster
        ]
        ...
        for main_idx in test:
            ...
            test_indices = [
                i
                for i in range(len(sorted_indices_of_cluster))
                if i != main_idx and not merged_flags[i]
            ]
            test_clusters = [positions_per_cluster[i] for i in test_indices]

            indices_merge = self.merge_test(
                main_cluster_direction, main_cluster_mean, test_clusters, average_dist
            )

            if indices_merge:
                clusters_to_merge = [main_idx] + [
                    test_indices[i] for i in indices_merge
                ]
                merge_indices_flattened = np.concatenate(
                    [sorted_indices_of_cluster[index] for index in clusters_to_merge]
                )
    ...
    def length(self, hits: np.ndarray) -> tuple:
        ...
        start_hit, end_hit = (
            hit_positions[np.min(indices)],
            hit_positions[np.max(indices)],
        )
    ...
    def clean_noise_hits(
        self, positions: np.ndarray, track_direction: np.ndarray, hits_mean: np.ndarray
    ) -> list:
        ...
        """
        projections = (
            np.dot(positions - hits_mean, track_direction[:, np.newaxis])
            * track_direction
            + hits_mean
        )
    ...
    def average_distance(
        self, positions: np.ndarray, hits_mean: np.ndarray, track_direction: np.ndarray
    ) -> float:
    ...
    # @staticmethod
    def select_muon_track(
        self, hits: np.ndarray, min_max_detector_bounds: np.ndarray
    ) -> tuple:
        """Test if cluster is a muon track.
            ...
            Returns:
                ...
                the average distance, and the direction.
            """
        muon_hits = []
        ...
        mask = self.clean_noise_hits(
            hit_positions, direction_vector, hits_mean_position
        )

        filtered_hits = hits[mask]

        avg_distance = self.average_distance(
            hit_positions[mask], hits_mean_position, direction_vector
        )
    ...
    def segments(self, muon_hits: np.ndarray) -> list:
        ...
        for hits in tpc_hits:
            if len(hits) != 0:
                hit_positions = np.array(
                    [[hit["x"], hit["y"], hit["z"]] for hit in hits]
                )
                ...
                for i in range(1, 1000):
                    ...
                    if segment_end[2] >= line_end[2]:
                        seg_info = self.grab_segment_info(
                            segment_end,
                            segment_start,
                            projected_hits,
                            hits,
    ...
    def grab_segment_info(
        self,
        segment_end: np.ndarray,
        segment_start: np.ndarray,
        projected_hits: np.ndarray,
        hits: np.ndarray,
        hit_ref: list,
        segment_to_track_ref: list,
    ) -> None | list:
        """Grab segment info.
        ...
        condition = (projected_hits[:, 2] >= min_bounds[2]) & (
            projected_hits[:, 2] <= max_bounds[2]
        )

        condition = (
            (projected_hits[:, 0] >= min_bounds[0])
            & (projected_hits[:, 0] <= max_bounds[0])
            & (projected_hits[:, 1] >= min_bounds[1])
            & (projected_hits[:, 1] <= max_bounds[1])
            & (projected_hits[:, 2] >= min_bounds[2])
            & (projected_hits[:, 2] <= max_bounds[2])
        )
        ...
        if len(hits_of_segment) != 0:
            ...
            drift_time = (
                max(hits_of_segment["t_drift"]) + min(hits_of_segment["t_drift"])
            ) / 2
    ...
    def run(self, file: str) -> tuple:
        ...
        Min_max_detector_bounds = [
            [min(self.z_boundaries), min(self.y_boundaries), min(self.x_boundaries)],
        ...
        for event in range(events.size):
            ...
            PromptHits_ev_positions = np.column_stack(
                (PromptHits_ev["x"], PromptHits_ev["y"], PromptHits_ev["z"])
            )

            nan_indices = np.unique(
                np.argwhere(np.isnan(PromptHits_ev_positions))[:, 0]
            )
            ...
            unique_points, counts = np.unique(
                PromptHits_ev_positions, axis=0, return_counts=True
            )
                ...
                for indices in hit_indices:
                    if len(indices) > 10:
                        ...
                        (
                            ...
                            avg_distance,
                            direction_vector,
                        ) = self.select_muon_track(hits, Min_max_detector_bounds)
                        ...
                        if len(muon_track) != 0:
                            ...
                            track_info = np.array(
                                [tuple(track_info)], dtype=self.rock_muon_dtype
                            )
                            muon_tracks.append(track_info)

                            # Get segments
                            segments_list, segment_hit_ref, segment_track_ref = (
                                self.segments(muon_track)
                            )
                            segments_array = np.array(
                                [tuple(segment) for segment in segments_list],
