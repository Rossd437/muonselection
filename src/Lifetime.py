...
def langau_func(x:np.ndarray, mpv:float, eta:float, sigma:float, A:float) -> np.ndarray:
...
def langau_lifetime(dNdx:np.ndarray, dqdx:np.ndarray, time_drifted:np.ndarray, time_bins:np.ndarray,
    ...
    print(f'Extracted lifetime of {params[1]} += {round(np.sqrt(cov[1,1]), 3)} microseconds')
     
    if plotting:
        #Lifetime plotting
        chi2 = chi_squared(params, ElectronLifetimeFunc, times[fit_points:], mpvs[fit_points:], mpv_uncertainties[fit_points:])
