...
def test_angle(my_selection, get_track_df, track_index):
    ...
    assert my_selection.angle(direction_vector) == (
        pytest.approx(track["theta_xz"], 1),
        pytest.approx(track["theta_yz"], 1),
        pytest.approx(track["theta_z"], 1),
    )
...
def test_close_to_two_faces(my_selection, get_track_df, track_index):
    ...
    mock_hits = np.array(
        [xyz_start, xyz_end], dtype=np.dtype([("x", "f8"), ("y", "f8"), ("z", "f8")])
    )

    min_boundaries = np.array(
        [
            my_selection.x_boundaries.min(),
            my_selection.y_boundaries.min(),
            my_selection.z_boundaries.min(),
        ]
    )
    max_boundaries = np.array(
        [
            my_selection.x_boundaries.max(),
            my_selection.y_boundaries.max(),
            my_selection.z_boundaries.max(),
        ]
    )
...
def test_average_distance(
    my_selection, positions, hits_mean, track_direction, expected_distance
):
    assert my_selection.average_distance(
        positions, hits_mean, track_direction
    ) == pytest.approx(expected_distance, 0.1)
...
def test_clean_noise_hits(
    my_selection, positions, track_direction, hits_mean, expected_hits
):
    assert (
        sum(my_selection.clean_noise_hits(positions, track_direction, hits_mean))
        == expected_hits
    )


@pytest.mark.parametrize(
    "hit_positions, expected_variance", [(np.array([(1, 1, 1), (3, 3, 3)]), 1.0)]
)
def test_PCAs(my_selection, hit_positions, expected_variance):
    results = my_selection.PCAs(hit_positions)
    assert results[0] == pytest.approx(expected_variance, 0.05)
