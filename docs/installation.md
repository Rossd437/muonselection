...
Now we can start on the conda environment. A script is needed to install instead of putting conda in the Snakefile because --no-build-isolation cannot be used with pip install in a conda environment yaml. There is a script in the `scripts` directory ready to be run that will install all the dependecies for you. Switch to the scripts directory with: <br>
